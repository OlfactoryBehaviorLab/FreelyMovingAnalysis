...
%Pose Data Directory
poseDataDir = uigetdir('D:\','Select Pose Data Folder:');
...
for z = 1:length(poseDataFiles)
    clearvars -except inputROI confidenceThreshold createVideo frames_to_average...
        ledLeftX ledRightX max_bad_frames ...
        z experimentDataDir poseDataDir videoDir poseDataFiles;
    
    error = -1;
    poseDataPath = strcat(poseDataFiles(z).folder,'\', poseDataFiles(z).name);
    fileStem = poseDataFiles(z).name(1:end-4);
    experiementDataPath = strcat(experimentDataDir, '\', fileStem, '.h5');
    videoPath = strcat(videoDir, '\', fileStem,'.mp4');
    
    
    opts = detectImportOptions(poseDataPath);
    opts = setvartype(opts,1,'double');
    try
        PoseData = readtable(poseDataPath,opts);
    catch
    ...
    if(PoseData.noseX(length(PoseData.index)) == 0)
        PoseData(length(PoseData.index),:) = [];
    end
    ...
    try
        ExperimentData = h5read(experiementDataPath,"/Trials");
    catch
